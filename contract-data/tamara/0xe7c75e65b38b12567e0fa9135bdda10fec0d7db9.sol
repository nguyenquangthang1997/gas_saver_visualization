...
contract ERCOTC is EventfulMarket, DSMath {
        ...
        returns (bool)
    {
        ...
        uint ethFee = offer.buy_amt / devFee;
        uint erc20Fee = offer.pay_amt / devFee;
        //send
        ...
        returns (bool)
    {
        ...
        uint ethFee = offer.pay_amt / devFee;
        uint erc20Fee = erc20Fee / devFee;
        //send
        require(erc20Interface.transferFrom(msg.sender, offer.owner, erc20Value), "Transfer failed");//send token to offer maker (seller)
        require(erc20Interface.transferFrom(msg.sender, devAddress, erc20Fee));//send token devfee
    ...
    {
        if(msg.value > 0){
            require(buyERC(uint256(id)), "Buy ERC failed");
        }
        else{
            require(buyETH(uint256(id)), "Sell ERC failed");
        }
