...
contract SuperFair is UtilFairWin, WhitelistAdminRole {
    ...
    function calStaticProfitInner(address userAddr) private returns(uint)
    {
        ...
        if(user.hisStaticAmount.add(user.hisDynamicAmount) >=  user.allInvest){
            ...
            for (uint i = user.staticFlag; i < user.invests.length; i++) {
                ...
                if(invest.day < 100) {
                    uint staticGaps = now.sub(invest.investTime).div(timeGaps.mul(1 minutes)); 
                    uint unlockDay = now.sub(invest.investTime).div(timeGaps.mul(1 minutes)); 
                    if (unlockDay>100) {
                        unlockDay = 100;
                        user.staticFlag++;
                    }
    ...
    function setTimeGaps(uint256 targetTimeGaps) external onlyWhitelistAdmin{
        timeGaps = targetTimeGaps;
    }
}
