...
contract ERC20 is Context, IERC20 {
    ...
    mapping (address => uint256) private _frozenAccounts;

    event FrozenFunds(address target, uint256 releaseTime);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        require(_frozenAccounts[sender] <= block.timestamp, "TokenTimelock: release time is before current time");
    ...
    function _freezeAccount(address account, uint256 releaseTime) internal {
        _frozenAccounts[account] = releaseTime;
        emit FrozenFunds(account, releaseTime);
    }
}
