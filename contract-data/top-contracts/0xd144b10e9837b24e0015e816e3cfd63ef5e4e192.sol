...
// File: baselib/Initializable.sol
...
// File: baselib/NativeMetaTransaction.sol
...
contract Robotars is ERC1155, ERC1155Holder, NativeMetaTransaction,Ownable {
    using SafeMath for uint256;
    ...
    function randomToken(uint16 curIndex) internal returns (uint16, uint16) {
        uint16 totalSize = MAX_TOTAL_SUPPLY - curIndex;
    ...
    function editPresale(
        ...
        for (uint16 i = 0; i < presaleAddresses.length; i++) {
            presaleWhitelist[presaleAddresses[i]] = amount[i];
        }
    }

    function mintPresale(uint16 _customBatchCount) external payable {
        uint16 reserved = presaleWhitelist[msg.sender];
        require(presaleActive, "Presale must be active to mint");
        require(reserved > 0, "No tokens reserved for this address");
        require(_customBatchCount <= reserved, "Can't mint more than reserved");
        ...
        presaleWhitelist[msg.sender] = reserved - _customBatchCount;
        _batchMint(msg.sender, _customBatchCount);
    }

    function mintPayable(uint16 _customBatchCount) external payable {
    ...
    function mint(address _to,uint16 _customBatchCount) external onlyOwner {
        if (_customBatchCount <= 0) _customBatchCount = 50;

        _batchMint(_to, _customBatchCount);
    }


    function _batchMint(address _to, uint16 num) private {
        uint16 totalSize = MAX_TOTAL_SUPPLY - currentID;        
        require(
            num <= totalSize,
            "Sorry, Request More than TotalSupply, Please Change Number"
        );
        uint16 curIndex = currentID;
        uint256[] memory ids = new uint256[](num);
        uint256[] memory amounts = new uint256[](num);
        
        for (uint16 i = 0; i < num; i++) {
            (ids[i], curIndex) = randomToken(curIndex);
            amounts[i] = 1;//MAX_NUM_PER_TOKEN;
        }
        currentID = curIndex;
    ...
    function setSalePreSale(bool isSale, bool isPreSale) public onlyOwner {
        presaleActive = isPreSale;
        saleActive = isSale;
    }    
