...
contract RoarStaking is Ownable, ERC721Holder, Pausable {
    using EnumerableSet for EnumerableSet.UintSet;

    IERC721 public roaringLeadersContractInstance;
    ...
    constructor(address roaringLeadersContractAddress, uint256 initialRate, uint256 numberOfExpirationBlocks, address roarTokenAddress) {
        roaringLeadersContractInstance = IERC721(roaringLeadersContractAddress);
        rate = initialRate;
    ...
    function setAddresses(address roaringLeadersContractAddress, address roarTokenAddress) public onlyOwner {
        roaringLeadersContractInstance = IERC721(roaringLeadersContractAddress);
        roarTokenContractInstance = IERC20(roarTokenAddress);
    }
    ...
    function deposit(uint256[] calldata tokenIds) external whenNotPaused {
        require(msg.sender != address(roaringLeadersContractInstance), "Invalid address");
        uint256 currentBlock = Math.min(block.number, expirationBlockNumber);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            roaringLeadersContractInstance.safeTransferFrom(msg.sender, address(this), tokenIds[i], "");
    ...
    function withdraw(uint256[] calldata tokenIds) external whenNotPaused {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            roaringLeadersContractInstance.safeTransferFrom(address(this), msg.sender, tokenIds[i], "");
        }
    }
