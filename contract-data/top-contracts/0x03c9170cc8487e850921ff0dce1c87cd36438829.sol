...
contract ThirdEyeApeSociety is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress;

    uint256 public TEAS_MAX = 11111;
    ...
    uint256 public PUBLIC_MINT_LIMIT = TEAS_MAX;
    ...
    uint256 private _activeDateTime = 1641952800; // (GMT): Wednesday, January 12, 2022 2:00:00 AM
    uint256 private _revealDateTime;
    
    string private _tokenBaseURI = "";
    string private _revealURI = "";
    
    Counters.Counter private _publicTEAS;

    constructor() ERC721("Third Eye Ape Society", "TEAS") {}
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicTEAS.current();

            if (_publicTEAS.current() < TEAS_MAX) {
                _publicTEAS.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicTEAS.current() < TEAS_MAX,"Purchase would exceed TEAS_MAX");
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory)
    {
        ...
        if (_mappingMintTimestamp[tokenId] + REVEAL_DELAY < block.timestamp && _revealDateTime < block.timestamp) {
