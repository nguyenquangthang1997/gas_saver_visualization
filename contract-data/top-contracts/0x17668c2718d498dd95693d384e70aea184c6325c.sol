...
contract TheCryminals is Ownable, ERC721 {
    uint constant maxPerWallet = 20;
    uint constant maxPerWalletPresale = 15;
    uint constant tokenPrice = .05 ether;
    uint constant maxSupply = 10000;
    uint public totalSupply = 0;
    uint8 public saleStatus = 0;
    address public referenceContract = 0x4F8730E0b32B04beaa5757e5aea3aeF970E5B613;
    ...
    function gift() public mutualCheck{
        require(winnerList[msg.sender], "can't claim a prize");
        winnerList[msg.sender] = false;
    ...
    function mint(uint _count) public payable mutualCheck{
        ...
        require(balanceOf(msg.sender) + _count <= maxPerWallet, "20 tokens per wallet");
        require(msg.value == tokenPrice * _count, "incorrect ether amount");
        if (saleStatus == 1){
            require(presaleList[msg.sender] || ERC721(referenceContract).balanceOf(msg.sender) > 0, "cannot purchase during presale");
            require(balanceOf(msg.sender) + _count <= maxPerWalletPresale, "15 tokens per wallet for presale");
        }
    ...
    function checkPresale() public view returns(bool){
        return presaleList[msg.sender] || ERC721(referenceContract).balanceOf(msg.sender) > 0;
    }
    ...
    function withdraw() public onlyOwner{
        ...
        payable(0x1fD88f9fe115Bcf91F1a523D8bB41a9b8D93Ce9A).transfer(_balance * 20 / 100);
        payable(0x772efb7fFC931Fc0EdC1A1a91e7C7F9B662da16c).transfer(_balance * 10 / 100);
        payable(0xb0EE3Cf26cC794ad7d901e24bc653D33d1956e8c).transfer(_balance * 14 / 100);
        payable(0xBa4b7a79fB24eC220d7a593AEA1cd107f0e0dB87).transfer(_balance * 14 / 100);
        payable(0x1Bcae4AAd9103029F925Ad3c2346b566ECF848c3).transfer(_balance * 14 / 100);
        payable(0xCCe267417F8138063b938809B9d7b0f0eFCbC720).transfer(_balance * 14 / 100);
