...
contract MadMeerkatBurrow is ERC721, Ownable, ReentrancyGuard {
    ...
    function mintMeerkat(uint numberOfTokens) external payable nonReentrant {
        require(saleIsActive); // Sale must be active
        require(numberOfTokens <= MAX_MEERKAT_PURCHASE); // Max mint of 20
        require(totalSupply().add(numberOfTokens) <= MAX_MEERKAT.sub(reservedMeerkats)); // Max supply exceeded
        require(meerkatPrice.mul(numberOfTokens) <= msg.value); // Value sent is not correct
    ...
    // @dev withdraw funds
    function withdrawBEP20(address token) external onlyOwner {
