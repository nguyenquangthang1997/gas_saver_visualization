...
contract MuleSquadNFT is ERC721Enumerable, Ownable { 
    
    uint public maxTokens = 690;
    uint public mintPrice = 0.069 ether;

    string internal baseTokenURI;
    string internal baseTokenURI_EXT;

    address internal shareAddress;

    bool public isPublicMinting = false;

    mapping(address => uint) public isWhitelisted;
    mapping(address => uint) public addressHasMinted;

    mapping(address => uint) public shareSign;

    event Mint(address indexed to, uint indexed tokenId);
    event AddToWhitelist(address whitelistedAddress);

    constructor() ERC721("MuleSquad", "MSQUAD") {}
    ...
    modifier onePerAddress() {
        require(addressHasMinted[msg.sender] == 0, "Only 1 mint per address!");
        _;
    }
    ...
    modifier publicMinting() {
        require(isPublicMinting, "Public claiming is not yet available!");
        _;
    }

    // funds withdrawal for owner
    function withdrawEther() external onlyOwner {
        require(shareAddress != address(0x0), "Share is not set");
        uint _balance = address(this).balance;
        uint _shares = _balance / 2;

        payable(shareAddress).transfer(_shares);
        payable(msg.sender).transfer(_shares);
    }

    function emergencyWithdrawEther() external onlyOwner {
        require(shareSign[shareAddress] == 1, "Share has not signed to accept this transaction.");
    ...
    function setShareAddress(address address_) external onlyOwner {
        require(shareAddress == address(0x0), "Address already set!");
        shareAddress = address_;
    }
    function signAsShare() external {
        require(msg.sender == shareAddress, "You are not Share");
        shareSign[msg.sender] = 1;
    }

    // whitelisting function
    function addToWhitelist(address[] memory addresses_) external onlyOwner {
        for (uint i = 0; i < addresses_.length; i++) {
            isWhitelisted[addresses_[i]] = 1;
            emit AddToWhitelist(addresses_[i]);
        }
    }
    ...
    function whitelistClaim() external onlySender onePerAddress isInWhitelist {
        require((totalSupply() + 1) <= maxTokens, "Not enough tokens!");

        addressHasMinted[msg.sender] = 1;
    ...
    function publicMint() payable external onlySender publicMinting {
