...
contract Regenz is ERC721Burnable, Ownable {

  uint256 constant public PRICE = 0.033 ether;
  uint256 constant public TOTAL_SUPPLY = 10000;
  uint256 constant public REVEAL_TIMESTAMP = 0;
  uint256 constant public MAX_PER_TX = 30;
  uint256 constant public NUM_TO_RESERVE = 250;

  uint256 public startingIndex;
  uint256 public startingIndexBlock;

  string constant public METADATA_HASH = "";
  ...
  mapping (address => RegenzBought) regenzBought;

  constructor(address payable _claimEthAddress) public ERC721("Regenz", "REGENZ") {
      claimEthAddress = _claimEthAddress;
  }

  function reserveRegenz() public onlyOwner {
      uint supply = totalSupply();
      uint i;
      for (i = 0; i < NUM_TO_RESERVE; i++) {
  ...
  * Mints Regenz
  */
  function mintRegenz(uint numberOfTokens) public payable {
      ...
      for(uint i = 0; i < numberOfTokens; i++) {
          ...
          regenzBought[msg.sender].regenz.push(mintIndex);
          regenzBought[msg.sender].wallet = msg.sender;
      }
  ...
  function getMintedRegenz(address wallet) public view returns (uint [] memory) {
       return regenzBought[wallet].regenz;
  }
