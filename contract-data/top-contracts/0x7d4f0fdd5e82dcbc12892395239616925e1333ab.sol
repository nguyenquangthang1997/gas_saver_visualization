...
contract MultiverseOfWomen is ERC721Enumerable, Ownable {
  using Strings for uint256;
  string baseURI;
  ...
  constructor(
   ...
   modifier onlyPresaleListed() {
        require(
            PresaleList[msg.sender]._canMint,
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount > 0,"Atleast mint more than 1 token");
    require(_mintAmount <= maxMintAmount,"Cannot mint more than 10 tokens.");
    require(supply + _mintAmount <= maxSupply,"Exceeds Supply");

    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount,"Make sure you sent the correct amount");
    }
    ...
    function presale(uint8 numberTokens) external payable onlyPresaleListed {
        ...
        require(supply + numberTokens <= maxSupply , "No More Tokens Left");
        require(supply+ numberTokens  <= preSaleMax,"Enough Tokens are not left!");
        
        require(numberTokens*PresalePrice <= msg.value,"Amount sent is less");
        for (uint256 i=1; i <= numberTokens; i++) {
            _safeMint(msg.sender, supply + i);
            preSaleMax = preSaleMax--;
        }
        PresaleList[msg.sender]._canMint = false;
