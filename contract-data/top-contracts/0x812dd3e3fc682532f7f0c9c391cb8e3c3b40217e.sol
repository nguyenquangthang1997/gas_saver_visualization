...
contract LiquidityMining is Ownable {
    ...
    struct PoolInfo {
        uint256 accBribePerShare;
        uint64 lastRewardBlock;
        uint64 end;
        bool locked;
        uint256 bribePerBlock;
        uint256 supply;
    }
    ...
    event LogPoolAddition(uint256 indexed pid, uint256 bribePerBlock, IERC20 indexed lpToken);
    event LogSetPool(uint256 indexed pid, uint256 bribePerBlock);
    event LogUpdatePool(
        uint256 indexed pid,
        uint64 lastRewardBlock,
        uint256 lpSupply,
    ...
    function add(
        ...
        uint256 maxRewards = uint256(end).sub(block.number).mul(bribePerBlock);
        require(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                locked: locked,
                bribePerBlock: bribePerBlock,
                supply: 0
            })
        );
        emit LogPoolAddition(lpToken.length.sub(1), bribePerBlock, _lpToken);
    }
    ...
    //     poolInfo[_pid].bribePerBlock = _instrumentalPerBlock;
    ...
    function pendingBribe(uint256 _pid, address _user) external view returns (uint256 pending) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint256 accBribePerShare = pool.accBribePerShare;
        if (block.number > pool.lastRewardBlock && pool.supply != 0) {
            ...
            uint256 sushiReward = blocks.mul(pool.bribePerBlock);
            accBribePerShare = accBribePerShare.add(
                sushiReward.mul(ACC_INST_PRECISION) / pool.supply
            );
        }
        pending = int256(user.amount.mul(accBribePerShare) / ACC_INST_PRECISION)
            .sub(user.rewardDebt)
            .toUInt256();
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lastRewardBlock < pool.end) {
            ...
            if (pool.supply > 0) {
                ...
                pool.accBribePerShare = pool.accBribePerShare.add(rewardFactor);
                pool.lastRewardBlock = uint64(lastValidBlock);
            }
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardBlock, pool.supply, pool.accBribePerShare);
        }
    }
    ...
    function deposit(
        ...
        user.rewardDebt = user.rewardDebt.add(
            int256(amount.mul(pool.accBribePerShare) / ACC_INST_PRECISION)
        );
    ...
    function harvest(uint256 pid, address to) public onlyUnlocked(pid) {
        ...
        int256 accumulatedInstrumental = int256(
            user.amount.mul(pool.accBribePerShare) / ACC_INST_PRECISION
        );
