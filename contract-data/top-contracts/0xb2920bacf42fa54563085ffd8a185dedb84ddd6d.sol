...
contract BattleApeGameCharacters is ERC721Enumerable, Ownable {
    ...
    uint256 public constant PUBLIC_SALE_COST = 0.1 ether;

    //max supply
    uint256 public constant PRIVATE_SALE_MAX_SUPPLY = 200;
    uint256 public constant PRE_SALE_MAX_SUPPLY = 200;
    uint256 public constant PUBLIC_SALE_MAX_SUPPLY = 3800;
    uint256 public constant GIVEAWAY_SUPPLY = 244;

    //max mint
    uint256 public privateMintLimitTx = 2;
    uint256 public maxPrivateMintLimit = 4;
    uint256 public presaleMintLimitTx = 2;
    uint256 public publicnMintLimitTx = 10;
    ...
    uint256 private _numAvailableTokens = 4444;
    uint256[4444] private _availableTokens;
    ...
    constructor() ERC721("Battle Ape Game - Characters", "AGC") {
        setBaseURI("ipfs://CID/");
    }

    function mint(uint256 _mintAmount) public payable {
        require(isPrivateSaleStart == true || isPresaleStart == true || isPublicSaleStart == true, "None of the sales is started yet!");
        require(_mintAmount > 0, "need to mint at least 1 NFT");

        if(isPrivateSaleStart == true){
            require(isWhitelisted[msg.sender]==true, "You're not whitelisted!");
            require(_mintAmount <= privateMintLimitTx, "You can mint in range (1-2) NFT!");
            require(privateMinted.add(_mintAmount)  <= PRIVATE_SALE_MAX_SUPPLY, "max NFT privatesale limit exceeded");
            require(mintedNFTs[msg.sender].add(_mintAmount) <= maxPrivateMintLimit, "You can mint max 4 NFTs!");
        ...
        else if(isPresaleStart == true){
            require(isWhitelisted[msg.sender]==true, "You're not whitelisted!");
            require(_mintAmount <= presaleMintLimitTx, "You can mint in range (1-2) NFT!");
            require(msg.value >= PRE_SALE_COST.mul(_mintAmount), "insufficient funds");
            require(presaleMinted.add(_mintAmount)  <= PRE_SALE_MAX_SUPPLY, "max NFT presalesale limit exceeded");
            ...
            require(_mintAmount <= publicnMintLimitTx, "You can mint in range (1-10) NFT!");
            require(msg.value >= PUBLIC_SALE_COST.mul(_mintAmount), "insufficient funds");
            require(publicMinted.add(_mintAmount)  <= PUBLIC_SALE_MAX_SUPPLY, "max NFT publicsale limit exceeded");
