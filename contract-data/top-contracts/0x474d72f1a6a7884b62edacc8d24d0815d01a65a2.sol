...
// File: contracts/utils/OwnerPausable.sol
...
// File: contracts/erc721/OpenSeaGasFreeListing.sol
...
// File: contracts/erc721/ERC721Common.sol
...
contract StrikeApes is ERC721Common, ReentrancyGuard {
    ...
    uint8 public constant MAX_PRESALE_MINT = 5;
    uint8 public constant MAX_MINT = 30;
    uint16 public constant MAX_ITEMS = 6_500;
    uint256 public PRESALE_PRICE = 0.032 ether;
    ...
    function presaleMint(address _to, uint16 _count) public payable 
        nonReentrant
        whenNotPaused {
        require(presaleActive == true, "Sale has not yet started");
        require(totalSupply() + _count <= MAX_ITEMS, "Max limit");
        require(presaleWhitelist[msg.sender] > 0, "You are not on white list");
        require(_count <= presaleWhitelist[msg.sender], "Exceeds number");
        require(msg.value >= PRESALE_PRICE * _count, "Value below price");

        presaleWhitelist[msg.sender] = presaleWhitelist[msg.sender] - _count;
    ...
    function mintReserve(address _to, uint16 _count) public onlyOwner {
        require(totalSupply() + _count <= MAX_ITEMS, "Sold out");
    ...
    function editToPresale(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            presaleWhitelist[addresses[i]] = MAX_PRESALE_MINT;
        }
    }
