...
interface IMerkleDistributor {
    ...
    function claim(uint256 index, address account, uint256 amount, uint256 time, bytes32[] calldata merkleProof) external;
    function claimAll(uint256[] calldata indexes, address account, uint256[] calldata amounts, uint256[] calldata times, bytes32[][] calldata merkleProofs) external;

    // This event is triggered whenever a call to #claim succeeds.
    event Claimed(uint256 index, address account, uint256 amount, uint256 time);
}
