...
contract Brainoids is ERC721Enumerable, Ownable {
    string baseURI;
    uint256 public constant MAX_BRAIN = 5555;
    uint256 public price = 55000000000000000;  //0.055 ETH
    uint256 public prePrice = 45000000000000000;  //0.045 ETH
    bool public publicSale = false;
    bool public preSale = false;
    mapping (address => uint256) public preMintAddrs;
    uint256 public preMintNums;
    address benefitAddr = 0x5eD624152EA66eE99fC50521430bf96Db69e1dfC;
    address devAddr = 0x3d0c83F3ABdEf9D19E2421Ed119A6C4557178c3B;
    ...
    function withdraw() public onlyOwner {
        payable(devAddr).transfer(address(this).balance*5/100);
        payable(benefitAddr).transfer(address(this).balance);
    }
    ...
    function ownerMint(uint nums) public onlyOwner{
        require(totalSupply() + nums < MAX_BRAIN, "sold out.");
        
        for(uint i = 0; i < nums; i++){
            uint id = totalSupply();
            if(id < MAX_BRAIN){
                _safeMint(0xe6519aB01cEc48F59465E65d18619737e6697D11, id);
            }
        }
    }
    ...
    function preMint(uint nums) external payable {
        ...
        require(preMintNums < 400, "ony 400 address can pre mint.");
        require(totalSupply() < MAX_BRAIN, "sold out.");
        require(totalSupply() + nums <= MAX_BRAIN, "not enough left.");
