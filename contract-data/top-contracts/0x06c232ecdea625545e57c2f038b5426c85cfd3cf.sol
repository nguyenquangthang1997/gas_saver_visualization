...
contract EtherMonkeyZombie is ERC721URIStorage, Ownable{
 
    event MintMonkeys (address indexed minter, uint256 startWith, uint256 times);
    ...
    function mintMonkey(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalMonkeys + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintMonkeys(_msgSender(), totalMonkeys+1, _times);
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalMonkeys + _times <= totalCount, "Mint amount will exceed total collection amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalMonkeys + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintMonkeys(_addr, totalMonkeys+1, 1);
        _mint(_addr, 1 + totalMonkeys++);
    } 
