...
/**
 * @title Mighty Manatee contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract MightyManatees is ERC721, Ownable {
    using SafeMath for uint256;

    uint256 public ManateePrice = 28000000000000000; // 0.028 ETH
    uint256 public MAX_MANATEES = 10000;
    uint256 public startBlockNumber;
    address payable public saleManager;
    uint256 public teamTokenCount = 0;
    uint256 public marketingTokenCount = 0;
    uint public claimWindow = 1600;
    uint public constant maxManateePurchase = 20;

    address public KZK;
    address public CROW;
    address public DF;

    mapping(address => uint) public freeManateeCount;
    ...
    constructor(uint _start, address _KZK, address _CRZ, address _DF) ERC721("Mighty Manateez", "MM") {
    ...
    function claimFreeManatee(address NFT, uint tokenID) external{
        require(block.number < startBlockNumber, "Can not claim manatees after the sale has started");
        require(block.number >= (startBlockNumber.sub(claimWindow)), 'Mighty Manatee: Can not claim manatees until claim window starts');
        require(NFT == CROW || NFT == KZK || NFT == DF, 'Mighty Manatee: NFT collection not eligible for free claim');
        require(!userAlreadyMintedWithNFTCollection[msg.sender][NFT], 'Mighty Manatee: Caller already used NFT collection to claim');
        require(!NFTAlreadyUsedForClaim[NFT][tokenID], 'Mighty Manatee: tokenID in NFT collection already used to claim');
        IERC721 nft = IERC721(NFT);
        require(nft.ownerOf(tokenID) == msg.sender, 'Mighty Manatee: Caller does not own tokenID in NFT collection');
        require(freeManateeCount[NFT].add(1) <= 100, 'Mighty Manatee: Max amount of free manatees minted for NFT collection');
        userAlreadyMintedWithNFTCollection[msg.sender][NFT] = true;
        NFTAlreadyUsedForClaim[NFT][tokenID] = true;
        freeManateeCount[NFT] = freeManateeCount[NFT].add(1);
        _safeMint(msg.sender, totalSupply());
    }

    function reserveManateesForTeam(uint amount, address to) public onlyOwner {
        require(block.number < startBlockNumber, "Can not reserve manatees after the sale has started");
        require(teamTokenCount.add(amount) <= 60, "Can not mint more than 60 tokens for the team");
    ...
    function reserveManateesForMarketing(uint amount, address to) public onlyOwner {
        require(block.number < startBlockNumber, "Can not reserve manatees after the sale has started");
        require(marketingTokenCount.add(amount) <= 100, "Can not mint more than 75 tokens for the team");
        marketingTokenCount = marketingTokenCount.add(amount);
        uint supply = totalSupply();
        uint i;
        for (i = 0; i < amount; i++) {
            _safeMint(to, supply + i);
    ...
    function changeNftPrice(uint256 price) public onlyOwner {
        ManateePrice = price;
    }
    ...
    function mintManatees(uint numberOfTokens) public payable {
        //want to add something to turn the sale off?
        require(block.number >= startBlockNumber, "Can not mint manatees before the sale has started");
        require(numberOfTokens <= maxManateePurchase, "Can only mint 20 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_MANATEES, "Purchase would exceed max supply of Manatees");
        require(ManateePrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");


        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_MANATEES) {
