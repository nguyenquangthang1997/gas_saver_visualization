...
contract AngryMeerkats is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Angry Meerkats","MEERKATS"){}
    ...
    function isClaimed (uint256 boar) public view returns (bool) {
        return _claimedBoars[boar];
    }

    function claim(uint256 angryBoarsTokensNumber) public {
        require(claimIsActive, "Claiming is not active at the moment");
        claimMeerkat(angryBoarsTokensNumber);
    }

    function claimByIds(uint256[] memory angryBoarIds) public {
        require(claimIsActive, "Claiming is not active at the moment");
    ...
    function claimMeerkat(uint256 angryBoarsTokensNumber) internal {
        require(angryBoarsTokensNumber > 0, "Provided value is not pozitive");
        ...
        require(unclaimedTokensCounter >= angryBoarsTokensNumber, "You do not possess enough claimable tokens");

        for (uint i = 0; i < walletBalance; i++) {
            if (!isClaimed(userTokens[i]) && angryBoarsTokensNumber > 0) {
                _safeMint(msg.sender, totalSupply());
                _claimedBoars[userTokens[i]] = true;
...
contract AngryBoarsContractInterface {
