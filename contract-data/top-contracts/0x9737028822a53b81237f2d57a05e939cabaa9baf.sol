...
contract ChillCubes is ERC721Enum, Ownable, ReentrancyGuard {
	using Strings for uint256;

	string public baseURI;
    string public baseExtension = ".json";

    IERC721 punkin = IERC721(0x48B8EF358f85179698cB5Ed134dC0E8151e9324f);
	uint256 public cost = 0.05 ether;
    uint256 public claimSupply = 666;
    ...
    function claimPunkin(uint[] calldata tokenId) public {
        for (uint256 i = 0; i < tokenId.length; ++i) {
            require(!claimed[tokenId[i]], "Exists");
            require(claim, "Off");
            require(punkin.ownerOf(tokenId[i]) == msg.sender, "Not Owner");
            uint256 s = totalSupply();
            require(s <= claimSupply, "Max" );
            _safeMint(msg.sender, s++, "" );
            delete s;
            claimed[tokenId[i]] = true;
        }
        claims = claims + tokenId.length;
    }
	function mintPresale(uint256 _mintAmount) public payable nonReentrant{
	uint256 s = totalSupply();
	uint256 reserve = presaleWhitelist[msg.sender];
	require(!status && !claim, "Off");
	...
	function gift() external onlyOwner{
	 uint256 s = totalSupply();
     uint256 remaining = claimSupply - claims;
