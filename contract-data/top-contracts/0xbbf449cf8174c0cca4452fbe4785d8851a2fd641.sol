...
contract Oink is ERC20("Oink", "OINK"), Ownable {
    ...
    constructor(address angryBoarsAddress, uint256 _baseRate) {
        iBoars = IBoars(angryBoarsAddress);
    ...
    function claimReward(uint256 _tokenId) public returns (uint256) {
        ...
        require(iBoars.ownerOf(_tokenId) == msg.sender, "Caller does not own the token being claimed for.");
    ...
    function burn(address _from, uint256 _amount) external {
		require(msg.sender == address(iSymbiosis));
		_burn(_from, _amount);
	}
	
	function setMeerk(address symbiosisAddress) public onlyOwner {
