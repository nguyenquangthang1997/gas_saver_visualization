...
contract RockETH is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "RockETH";
    string constant _symbol = "ROCKETH";
    ...
    uint256 additionalSellFee = 1;

    address public autoLiquidityReceiver=0x69F0EB8c7770C0845782EF20402D4ee24d3f106c;
    address public marketingFeeReceiver=0x69F0EB8c7770C0845782EF20402D4ee24d3f106c;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
       require(sender == owner || recipient == owner || launched(), "Not launched yet");
       require(!blacklist[sender], "You are blacklisted");

       if(whitelistOnly) {
           if(!whitelist[recipient] && sender == pair) {
...
//        emit Transfer(sender, recipient, amount);
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling){ return totalFee.add(additionalSellFee); }
        return totalFee;
    }
    ...
    function setAdditionalSellFee(uint256 value) external authorized {
        require(value <= 15, "value too high");
        additionalSellFee = value;
    }
    ...
    function removeWhitelistOnly() external authorized {
        whitelistOnly = false;
    }

    function multiWhitelist(address[] memory addresses, bool _bool) external authorized {
        for (uint256 i = 0;i < addresses.length; i++){
            whitelist[addresses[i]] = _bool;
        }
    }

    function multiBlacklist(address[] memory addresses, bool _bool) external authorized {
        for (uint256 i = 0;i < addresses.length; i++){
