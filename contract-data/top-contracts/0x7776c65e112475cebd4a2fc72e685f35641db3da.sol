...
contract StakingManager is TokenSaver {
    ...
    modifier onlyGov() {
        require(hasRole(GOV_ROLE, _msgSender()), "StakingManager.onlyGov: permission denied");
        _;
    }
    ...
    modifier onlyRewardDistributor() {
        require(
            ...
            "StakingManager.onlyRewardDistributor: permission denied"
        );
        _;
    }

    modifier onlyGovOrRewardDistributor() {
        require(
            hasRole(GOV_ROLE, _msgSender()) || hasRole(REWARD_DISTRIBUTOR_ROLE, _msgSender()),
            "StakingManager.onlyGovOrRewardDistributor: permission denied"
        );
        _;
    }
    ...
    constructor(address _reward, address _rewardSource) {
        require(_reward != address(0), "StakingManager.constructor: reward token must be set");
        require(_rewardSource != address(0), "StakingManager.constructor: rewardSource token must be set");
    ...
    function addPool(address _poolContract, uint256 _weight) external onlyGov {
        distributeRewards();
        require(_poolContract != address(0), "StakingManager.addPool: pool contract must be set");
        require(!poolAdded[_poolContract], "StakingManager.addPool: Pool already added");
        require(pools.length < MAX_POOL_COUNT, "StakingManager.addPool: Max amount of pools reached");
        // add pool
    ...
    function removePool(uint256 _poolId) external onlyGov {
        require(_poolId < pools.length, "StakingManager.removePool: Pool does not exist");
        distributeRewards();
    ...
    function adjustWeight(uint256 _poolId, uint256 _newWeight) external onlyGov {
        require(_poolId < pools.length, "StakingManager.adjustWeight: Pool does not exist");
        distributeRewards();
    ...
    /**
     * @dev distribute token to Pool(s)
     *
     * Requirements:
     * - LiquidityMiningManager must be approved by rewardSource
     */
    function distributeRewards() public onlyGovOrRewardDistributor {
