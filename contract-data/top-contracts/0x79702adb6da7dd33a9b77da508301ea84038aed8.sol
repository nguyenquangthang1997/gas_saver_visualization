...
contract WheeliesNFT is ERC721Enumerable, Ownable {
    
    using SafeMath for uint256;
    
    bool public mintingActive;
    uint256 public mintingPrice = .05 ether;
    uint256 public mintingPerTxLimit = 5;
    uint256 public mintingStartTimestamp;
    mapping(address => uint256) public numPurchased;
    mapping(address => bool) public whitelist;
    address public poe;
    uint256 public POEBuffer;
    ...
    constructor(uint256 mintingStart, string memory _uri, address poeToken, uint256 buffer) ERC721("Wheelies", "WHEEL") {
        mintingActive = true;
    ...
    function addWhitelist(address buyer) public onlyOwner{
        whitelist[buyer] = true;
    }

    function addManyWhitelist(address[] calldata buyers) public onlyOwner{
        for (uint i = 0; i < buyers.length; i++){
            whitelist[buyers[i]] = true;
        }
    }

    function removeWhitelist(address ape) public onlyOwner{
        whitelist[ape] = false;
    }

    function removeManyWhitelist(address[] calldata buyers) public onlyOwner{
        for (uint i = 0; i < buyers.length; i++){
    ...
    function mint(uint256 _amount) public payable {
        ...
        require(mintingStartTimestamp <= block.timestamp || 
            ((IERC20(poe).balanceOf(msg.sender) > 0 || isWhitelisted(msg.sender)) &&
            block.timestamp + POEBuffer >= mintingStartTimestamp), "Sale not started yet"); //Introduce presale for whitelist + POE holders
    ...
    */
     ...
     */
    function withdrawFunds(address client) public onlyOwner {
        uint balance = address(this).balance;
        uint accounting = balance.mul(1000);
        uint toClient = accounting.div(1177); //accounting magick to assign 85% to client
        uint toOwner = balance.sub(toClient);
        payable(msg.sender).transfer(toOwner);
        payable(client).transfer(toClient);
    }
