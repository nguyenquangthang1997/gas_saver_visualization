...
contract Primera is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    uint public constant MAX_TOKENS = 400;
    bool public hasSaleStarted = false;
    mapping(uint256 => uint256) public creationDates;
    mapping(uint256 => address) public creators;
    string public METADATA_PROVENANCE_HASH = "";
    string public GENERATOR_ADDRESS = "https://mystudios.io/pieces/primera.php?number=";
    uint256 private price = 90000000000000000;
    mapping(uint256 => string) public script;

    constructor() ERC721("Primera by Mitchell and Yun", "PRMA")  {
        setBaseURI("https://mystudios.io/api/primera.php?number=");
    ...
    function mintToken(uint256 maxTOKENS) public payable {
        ...
        require(msg.value >= getPrice().mul(maxTOKENS), "Ether value sent is below the price");
    ...
    function addScriptChunk(uint256 index, string memory _chunk) public onlyOwner {
        script[index]=_chunk;
    }

    function getScriptChunk(uint256 index) public view returns (string memory){
        return script[index];
    }

    function removeScriptChunk(uint256 index) public onlyOwner {
        delete script[index];
    }
