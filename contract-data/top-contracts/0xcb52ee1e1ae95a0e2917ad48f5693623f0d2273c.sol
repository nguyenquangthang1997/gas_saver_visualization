...
// File contracts/DarkZodiacCollectible.sol
...
contract DarkZodiacCollectible is ERC721, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    using SafeMath for uint256;
    string private _tokenBaseURI = "https://meta.darkzodiac.com/";
    string public DARKZ_PROVENANCE = "";
    
    uint256 public constant zodiacPrice = 0.09 ether;
    ...
    uint256 public RESERVED_ZODIAC = 50;
    uint256 public MAX_ZODIACS = 3876;
    ...
    constructor () public ERC721 ("DarkZodiac", "DARKZ"){
    }
    
    function reserveZodiac() public onlyOwner {
        require(!reserved, "Already reserved");

        reserved = true;

        uint i;
        for (i = 0; i < RESERVED_ZODIAC; i++) {    
            _tokenIds.increment();
    ...
    */
    ...
    function mintZodiac(uint256 tokenQuantity) external payable {
        require(saleLive, "Sale must be active to mint Zodiac");
        require(!presaleLive, "ONLY_PRESALE");
        require(zodiacPrice * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
        require(_tokenIds.current() + tokenQuantity <= MAX_ZODIACS, "Purchase would exceed max supply of Zodiac");
        require(tokenQuantity <= purchaseLimit, "EXCEED_ALLOC");
        require(_tokenIds.current() < MAX_ZODIACS, "OUT_OF_STOCK");
    ...
    function presaleBuy(uint256 tokenQuantity) external payable {
        ...
        require(_tokenIds.current() + tokenQuantity <= MAX_ZODIACS, "OUT_OF_STOCK");
