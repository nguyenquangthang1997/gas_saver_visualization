...
$$ |         $$ |   $$ |            $$ |  $$ |$$   ____|$$ |  $$ |$$ |
$$$$$$$$\    \__|   $$$$$$$$\       \$$$$$$  |\$$$$$$$\ $$ |  $$ |$$$$$$$$\
\________|          \________|       \______/  \_______|\__|  \__|\________|
****************************************************************************/
...
// File @openzeppelin/contracts/token/ERC721/extensions/IERC721Enumerable.sol@v4.3.3
...
// File @openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol@v4.3.3
...
// File contracts/TwoPlusTwoGen2.sol

pragma solidity ^0.8.0;


contract TwoPlusTwoGen2 is ERC721Enumerable, Ownable {
    ...
    constructor(address genesis) ERC721("TwoPlusTwoGen2", "TPT2") {
        _genesis = genesis;
    }

    function checkAirdropByAddress(address owner) public view returns (uint256) {
        ERC721Enumerable nfts = ERC721Enumerable(_genesis);
        uint256 nftCount = nfts.balanceOf(owner);
    ...
    function checkAirdropByTokenId(uint256 nftId) public view returns (bool) {
        require(nftId < (maxSupply - 1), "Invalid Token ID");
    ...
    function claimAirdrop(uint256 limit) public {
        require(isActive, "Minting not active");

        ERC721Enumerable nfts = ERC721Enumerable(_genesis);
        uint256 nftCount = nfts.balanceOf(msg.sender);

        require(nftCount > 0, "Address must own a genesis NFT");
    ...
    function claimAirdropForOwner(address owner) public onlyOwner {
        ERC721Enumerable nfts = ERC721Enumerable(_genesis);
        uint256 nftCount = nfts.balanceOf(owner);
        ...
        for (uint256 i = 0; i < nftCount; i++) {
            uint256 nftId = nfts.tokenOfOwnerByIndex(owner, i);
            // If this id has already been minted, just continue
            if (_exists(nftId)) {
                continue;
            }
