...
contract IMETACoin223Token_13 is ERC20, ERC223, Standard223Receiver, SafeMath {
...
//from ExampleToken
    ...
    constructor(/*uint initialBalance*/) {
        ...
        name        = "iMETA Coin";                     // Set the name for display purposes
        symbol      = 'IMC';                            // Set the symbol for display purposes

        uint initialBalance  = (10 ** uint256(decimals)) * 10000*1000*1000;
        ...
        oneTransferLimit    = (10 ** uint256(decimals)) * 10000*1000*1001;   // to simulate no limit
        oneDayTransferLimit = (10 ** uint256(decimals)) * 10000*1000*1001;   // to simulate no limit
...
//from StandardToken
    ...
    function super_transferFrom(address _from, address _to, uint _value) /*public*/ internal returns (bool success) {
        ...
        if(msg.sender == contrInitiator && _from == thisContract) {
            ...
            allowed[_from][msg.sender] = safeSub(loc_allowance, _value);
        }
...
//              if(block.timestamp - transferInfo[_from].ti[i].time < 10 minutes) {
//              if(block.timestamp - transferInfo[_from].ti[i].time < 1 hours) {
                if(block.timestamp - transferInfo[_from].ti[i].time < 1 days) {
