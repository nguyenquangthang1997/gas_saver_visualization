...
contract OtherworldlyFreaks is ERC721Enumerable, Ownable{
    using Strings for uint256;
    ...
    uint256 public constant NFT_FREAK_PRICE = 0.05 ether;
    
    uint256 public constant MAX_TOKEN = 3;
    
    bool public saleActivated;

    constructor()
        ERC721("Otherworldly Freaks", "OFR")
    {
    }
    ...
    function Mint(uint256 _amount) public payable {
        require(saleActivated, "Mint: Mint process is pause");
        
        require(_amount > 0, "Mint: amount cannot be 0");
        require(totalSupply() < TOTAL_SUPPLY, "Mint: sale has already ended");
        require(totalSupply().add(_amount) <= TOTAL_SUPPLY, "Mint: sale has already ended");
        require(_amount <= MAX_TOKEN, "Mint: You may not buy more than MAX_TOKEN NFTs at once");
        require(msg.value == _amount.mul(NFT_FREAK_PRICE), "Mint: Ether value sent is not correct");
    ...
    function assignNFT(address[] calldata destinations, uint256 _amount) public onlyOwner{
        ...
        require(_amount <= MAX_TOKEN, "assignNFT: You may not buy more than MAX_TOKEN NFTs at once");
