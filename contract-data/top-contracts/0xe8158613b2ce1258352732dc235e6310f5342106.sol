...
Vasel Inu Socials:
Telegram: https://t.me/vasel_inu
Twitter: https://twitter.com/Vasel_Inu
Website: https://www.vasel-inu.com
...
contract VaselInu is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 1000000000000 * 10**9;
    string private m_Name = "Vasel Inu";
    string private m_Symbol = "VINU";
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != address(0) && _recipient != m_UniswapV2Pair && block.timestamp <= m_Launched.add(876000 hours);
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(1440 minutes))
            return TOTAL_SUPPLY.div(80);
        else
            return TOTAL_SUPPLY;
    }
