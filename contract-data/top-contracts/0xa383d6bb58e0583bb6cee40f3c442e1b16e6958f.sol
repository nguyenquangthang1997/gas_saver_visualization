...
contract E1337Wearable is ERC721 {
    ...
    constructor() ERC721("E1337", "E1337") {}
    ...
    function retrieve(uint256 _id) external view returns (WearableView memory) {
        for (uint256 i; i < models.length; i++) {
    ...
    function retrieveModelByName(string memory _name)
        external
        view
 ...
 * {ERC721Enumerable}.
 */
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function ownerOf(uint256 tokenId)
        ...
        returns (address)
    {
        require(_exists(tokenId), "ERC721: ownerOf for nonexistent token");

        address ownerAddress = _owners[tokenId];

        // If there is no owner set, consider the owner of the contract as the owner.
    ...
    function _increaseBalance(address to, uint256 quantity) internal virtual {
