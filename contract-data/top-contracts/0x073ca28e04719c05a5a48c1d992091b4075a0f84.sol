...
@title  RADIOACTIVEPUNKS

@dev    Extends ERC721 Non-Fungible Token Standard to allow pausing, burning,
        enumerability, and royalties via the Rarible V1 Protocol and ERC2981 NFT
        Royalty Standard. The metadata and images are stored on Arweave,
        creating a completely on-chain generative collectible series. If any
        NFTs were to survive nuclear winter, these would be the ones. Tokens
        are only mintable by a signed EIP-718 transaction to ward off contract
        attacks and minting via Etherscan. Any singular wallet can mint only ten
        in the first 10 mins of sales and only 100 lifetime.
*/
...
contract RADIOACTIVEPUNKS is ERC721Burnable, ERC721Pausable, ERC721Enumerable, HasSecondarySaleFees, ERC2981, Ownable {
  ...
  uint256 public activationTime = 1633536000; // THE CRYPTOPOCALYPSE
  ...
  uint256 public constant punkPrice = 25000000000000000; // 0.025 ETH
  uint private constant maxPunksAtOnce = 5;
  uint256 private MAX_PUNKS = 3000;
  uint256 public PUNKS_LEFT = 2999;
  uint256 private PRIME = 197;
  uint256 private COPRIME = 474569;
  ...
  constructor() ERC721("Radioactive Punks", "RPUNK") {}
  ...
  function setActivationTime(uint256 timestampInSeconds) external onlyOwner {
    activationTime = timestampInSeconds;
  }

  /*
   * @dev Used in pseudorandom tokenId
   */
  function setCoprimes(uint256 primeNumber, uint256 coprimeNumber) external onlyOwner {
    PRIME = primeNumber;
    COPRIME = coprimeNumber;
  }
   ...
   */
  function reservePunks(uint256 numberOfTokens) public onlyOwner {
    require(numberOfTokens <= PUNKS_LEFT, "Exceeds max supply");

    // for every tokent they want
    for(uint i = 0; i < numberOfTokens; i++) {
      // reduce the total pop after minting
      if (PUNKS_LEFT >= 0) {
        uint randomUnusedInt = ((PRIME * PUNKS_LEFT) + COPRIME) % MAX_PUNKS;
        _safeMint(msg.sender, randomUnusedInt);
        PUNKS_LEFT--;
      }
    }
  }
  ...
  /*
   * For minting up to five punks at once - tokenIds pseudorandomly generated.
   */
  function mintPunks(uint256 numberOfTokens, uint8 sigV, bytes32 sigR, bytes32 sigS) public payable {
    require(activated(), 'You are too early');
    require(numberOfTokens <= maxPunksAtOnce, "Exceeds max mint rate");
    require(numberOfTokens <= PUNKS_LEFT, "Exceeds max supply");
    require(punkPrice * numberOfTokens == msg.value, "Ether value incorrect");
    require(balanceOf(msg.sender) < 100, "Owns max amount");
    require(verify(msg.sender, sigV, sigR, sigS) == true, "No cheaters!");

    if (block.timestamp <= activationTime + 600) {
      require(balanceOf(msg.sender) + numberOfTokens <= 10, "Only 10 for first 10m");
    }

    // for every token they want
    for(uint i = 0; i < numberOfTokens; i++) {
      // reduce the total pop after minting
      if (PUNKS_LEFT >= 0) {
        uint randomUnusedInt = ((PRIME * PUNKS_LEFT) + COPRIME) % MAX_PUNKS;
  ...
  function supportsInterface(bytes4 interfaceId) public view override(ERC721, HasSecondarySaleFees, ERC721Enumerable, ERC2981) returns (bool) {
