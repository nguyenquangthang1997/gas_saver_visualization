...
contract PonyoInu is Context, IERC20, Ownable {
    ...
    uint256 public airDropDailySellPerc = 5;
    ...
    uint256 private gasPriceLimit = 498 * 1 gwei; // do not allow over x gwei for launch
    ...
    constructor() payable {
        _rOwned[_msgSender()] = _rTotal / 1000 * 250;
        _rOwned[address(this)] = _rTotal / 1000 * 750;

        airDropLimitLiftDate = block.timestamp + 14 days;
        ...
        marketingAddress = payable(0x793c894c633FE539f8D226eB41C36C651bE5c67F); // Marketing Address
        
        devAddress = payable(0x6B2c5a6df5E51264e31479605b55bD73b727Bbf5); // Dev Address
        ...
        emit Transfer(address(0), _msgSender(), _tTotal * 250 / 1000);
        emit Transfer(address(0), address(this), _tTotal * 750 / 1000);
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyCharityFee;
                buyOrSellSwitch = BUY;
            } 
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellCharityFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 ethForDev= ethForMarketing * 2 / 5;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyCharityFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyCharityFee <= 10, "Must keep buy taxes below 10%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellCharityFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellCharityFee <= 15, "Must keep sell taxes below 15%");
