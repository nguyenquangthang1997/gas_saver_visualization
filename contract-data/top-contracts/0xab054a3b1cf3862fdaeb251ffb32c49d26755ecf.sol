...
contract MechaPenguins is ERC721Delegated {
    uint256 public constant MAX_PURCHASE_PER_TX = 20;

    uint256 public MAX_SUPPLY = 13332;
    uint256 public reserveTokenId = 8888;
    ...
    function claimPenguins(uint256[] memory nftTokenIds, address[] memory contractAddresses) public {
    ...
    function addToAllowedContracts(address[] memory contractAddresses) public onlyOwner {
        for(uint i = 0; i < contractAddresses.length; i++) {
            allowedContracts[contractAddresses[i]] = true;
        }
    }

    function removeFromAllowedContracts(address[] memory contractAddresses) public onlyOwner {
        for(uint i = 0; i < contractAddresses.length; i++) {
    ...
    function setClaim(bool shouldClaim) external onlyOwner {
        openClaim = shouldClaim;
    }
