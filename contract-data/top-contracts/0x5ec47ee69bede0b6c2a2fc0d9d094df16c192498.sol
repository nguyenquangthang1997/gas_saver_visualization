...
contract ShibLevSwapper is IUniswapV3SwapCallback {
    using BoringERC20 for IERC20;
    using SafeCast for uint256;
    using BoringMath for uint256;
    ...
    function swap(
        ...
        bool zeroForOne = address(WETH) < address(SHIB);
        ...
        (, shareReturned) = bentoBox.deposit(SHIB, address(bentoBox), recipient, amountTo, 0);
        extraShare = shareReturned.sub(shareToMin);
    }
}
