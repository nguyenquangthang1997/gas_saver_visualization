...
contract PUREBLOOD is ERC20, Ownable {
    ...
    constructor() ERC20("PUREBLOOD", "PUREBLOOD") {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CJP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && automatedMarketMakerPairs[to]) {
        	uint256 _liquidityfees = amount.mul(liquidityFee).div(100);
            uint256 _charityfees = amount.mul(charityFee).div(100);
        	amount = amount.sub(_liquidityfees.add(_charityfees));
            super._transfer(from, address(this), _liquidityfees); 
            super._transfer(from, charityWallet, _charityfees);
