...
contract TenaciousTigers is ERC721Enumerable, Ownable {
    ...
    uint256 private reserved = 109;
    uint256 public mintCost = 0.06 ether; 
    uint256 private maxPublicSupply = 10000;
    uint256 public MAX_PURCHASEABLE = 20;
    uint256 public presaleStartTime = 1635350400 - 2 days; 
    uint256 public publicStartTime = presaleStartTime + 2 days;
    uint256 public reserveSent = 0;
    string public PROVENANCE = "";


    constructor(string memory baseTokenURI) ERC721("TenaciousTigers", "TT")  {
    ...
    function claimPublicSale(
        ...
        require( block.timestamp >= publicStartTime,                                "Presale has not started yet" );
        require( totalSupply() + _amountToMint - reserveSent < maxPublicSupply,     "Exceeds supply" );
        require( msg.value >= mintCost.mul(_amountToMint),                          "Insufficient Ether" );

        // Verify that amount is 20 or less
        require(_amountToMint < 21, "Max 20");
        for(uint256 i; i < _amountToMint; i++){
            uint256 supply = totalSupply();
            _safeMint( msg.sender, supply );
        }
    ...
    function reserves(address _to, uint256 _amountToMint) external onlyOwner() {
        
        require( _amountToMint <= reserved, "Exceeds reserved supply" );
        
        for(uint256 i = 0; i < _amountToMint; i++){
            _safeMint( _to, maxPublicSupply + reserveSent + i );
        }
        reserveSent += _amountToMint;
        reserved -= _amountToMint;
    }

    function withdrawal() public payable onlyOwner {
        address a1 = 0x27859BD4747aCaE8262931d99973D8AD5F331Cd0;
        address a2 = 0xE8F3FD6b60E0a25DC9Fe096a70428153B7a0dC08;

        uint256 bal1 = address(this).balance.mul(13125).div(100000);
        uint256 bal2 = address(this).balance.sub(bal1);
