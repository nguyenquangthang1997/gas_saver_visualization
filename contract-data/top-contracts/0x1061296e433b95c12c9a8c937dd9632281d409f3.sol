...
/**
 ...
 .----------------.  .----------------.  .----------------.  .-----------------. .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |   ______     | || |   _____      | || | _____  _____ | || | ____  _____  | || |  ________    | || |  _________   | || |  _______     | |
| |  |_   __ \   | || |  |_   _|     | || ||_   _||_   _|| || ||_   \|_   _| | || | |_   ___ `.  | || | |_   ___  |  | || | |_   __ \    | |
| |    | |__) |  | || |    | |       | || |  | |    | |  | || |  |   \ | |   | || |   | |   `. \ | || |   | |_  \_|  | || |   | |__) |   | |
| |    |  ___/   | || |    | |   _   | || |  | '    ' |  | || |  | |\ \| |   | || |   | |    | | | || |   |  _|  _   | || |   |  __ /    | |
| |   _| |_      | || |   _| |__/ |  | || |   \ `--' /   | || | _| |_\   |_  | || |  _| |___.' / | || |  _| |___/ |  | || |  _| |  \ \_  | |
| |  |_____|     | || |  |________|  | || |    `.__.'    | || ||_____|\____| | || | |________.'  | || | |_________|  | || | |____| |___| | |
| |              | || |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 
...
// Plunder for pirates, an essential cache of 5000 limited NFT equipment for the everyday swashbuckler sailing the opensea, mint a pirate now at https://ploddingpirates.com",
...
contract Plunder is ERC721Enumerable, Ownable {
    ...
    uint public constant MAX_NFT_PURCHASE = 11; // One free NFT plus purchase of 10 extra
    uint256 public MAX_PUBLIC_SUPPLY = 4000;
    uint256 public MAX_RESERVE_SUPPLY = 100;
    bool public saleIsActive = false;
    uint public startTimestamp = 1632130200; // Tuesday, September 21, 2021 22:00 UTC
    ...
    function withdraw() external {
        require(
                ...
                msg.sender == three_address 
        );

        uint256 bal = address(this).balance;

        uint256 FortyFive = bal.mul(45).div(100);
        (bool sent1,) = one_address.call{value: FortyFive}("");
        require(sent1, "Failed to send Ether");

        uint256 FortyFive1 = bal.mul(45).div(100);
        (bool sent2,) = two_address.call{value: FortyFive1}("");
        require(sent2, "Failed to send Ether");

        uint256 Ten = bal.mul(10).div(100);
        (bool sent3,) = three_address.call{value: Ten}("");
        require(sent3, "Failed to send Ether");
        
         uint256 remaining = address(this).balance;
        (bool sent4,) = one_address.call{value: remaining}("");
