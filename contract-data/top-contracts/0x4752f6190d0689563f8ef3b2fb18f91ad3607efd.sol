...
contract WGMarketplace is Ownable, ReentrancyGuard {
    ...
    // 6.5% creator royalty goes to the WG deployer address
    uint256 public fee;
    address public feeRecipient;

    // Community decides what happens with service fees, for now service fees = 0%
    ...
    event UpdateServiceFeeRecipient(address feeRecipient);
        ...
        validListing(_nftAddress, _tokenId, _owner)
    {
        ...
        WGOracle(oracle).reportSale(_nftAddress, _tokenId, paymentToken, listedItem.pricePerItem);
    ...
    function _buyItem(
        ...
        uint256 serviceFeeAmount = totalPrice * serviceFee / BASIS_POINTS;
        IERC20(paymentToken).safeTransferFrom(_msgSender(), feeRecipient, feeAmount);
        IERC20(paymentToken).safeTransferFrom(_msgSender(), serviceFeeRecipient, serviceFeeAmount);
        IERC20(paymentToken).safeTransferFrom(_msgSender(), _owner, totalPrice - feeAmount - serviceFeeAmount);
    }
    ...
    function setServiceFeeRecipient(address _feeRecipient) public onlyOwner {
        serviceFeeRecipient = _feeRecipient;
        emit UpdateServiceFeeRecipient(_feeRecipient);
    }
    ...
    function setOracleOwner(address _newOwner) public onlyOwner {
        WGOracle(oracle).transferOwnership(_newOwner);
    }
