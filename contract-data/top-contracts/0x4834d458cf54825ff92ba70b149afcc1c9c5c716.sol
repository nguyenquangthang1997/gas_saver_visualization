...
/**
 * @title Nft
 * Nft - a contract for my non-fungible creatures.
 */
contract Nft is ERC721Tradable {
    constructor(address _proxyRegistryAddress, address _revealer, string memory _baseTokenURI)
        ERC721Tradable("The Karma Chameleons", "TKC", _proxyRegistryAddress, _revealer, _baseTokenURI)
    {}
}
...
contract Factory is FactoryERC721, Ownable {
    ...
    uint256 NFT_SUPPLY = 10_000;

    /*
     * Three different options for minting Nfts (basic, premium, and gold).
     */
    uint256 NUM_OPTIONS = 3;
    uint256 SINGLE_CREATURE_OPTION = 0;
    uint256 MULTIPLE_CREATURE_OPTION_1 = 1;
    uint256 MULTIPLE_CREATURE_OPTION_2 = 2;
    uint256 NUM_CREATURES_IN_MULTIPLE_CREATURE_OPTION_1 = 5;
    uint256 NUM_CREATURES_IN_MULTIPLE_CREATURE_OPTION_2 = 10;
    ...
    function name() override external pure returns (string memory) {
        return "The Karma Chameleons Eggs Hatching";
    }
    ...
    function mint(uint256 _optionId, address _toAddress) override public {
        ...
        Nft openSeaNft = Nft(nftAddress);
        if (_optionId == SINGLE_CREATURE_OPTION) {
            openSeaNft.mintTo(_toAddress);
        } else if (_optionId == MULTIPLE_CREATURE_OPTION_1) {
            for (
                uint256 i = 0;
                i < NUM_CREATURES_IN_MULTIPLE_CREATURE_OPTION_1;
                i++
            ) {
                openSeaNft.mintTo(_toAddress);
            }
        } else if (_optionId == MULTIPLE_CREATURE_OPTION_2) {
            for (
                uint256 i = 0;
                i < NUM_CREATURES_IN_MULTIPLE_CREATURE_OPTION_2;
                i++
            ) {
                openSeaNft.mintTo(_toAddress);
            }
    ...
    function canMint(uint256 _optionId) override public view returns (bool) {
        ...
        Nft openSeaNft = Nft(nftAddress);
        uint256 nftSupply = openSeaNft.totalSupply();

        uint256 numItemsAllocated = 0;
        if (_optionId == SINGLE_CREATURE_OPTION) {
            numItemsAllocated = 1;
        } else if (_optionId == MULTIPLE_CREATURE_OPTION_1) {
            numItemsAllocated = NUM_CREATURES_IN_MULTIPLE_CREATURE_OPTION_1;
        } else if (_optionId == MULTIPLE_CREATURE_OPTION_2) {
            numItemsAllocated = NUM_CREATURES_IN_MULTIPLE_CREATURE_OPTION_2;
        }
        return (nftSupply + numItemsAllocated) <= NFT_SUPPLY;
    }
