...
Most mongoose speciare are highly social animals living in
busy groups of 6 to 40 individuals called "packs" or "mobs".
Some particularly gregarious species like the Gambian mongoose
may live in packs of 50 ore more.

The $MONG team has come together with the leaders of its community to 
release C4G, a FAAS (Farming As a Service) model for Mongoose. Keeping with 
Mongoose's mission of giving back to those less fortunate, we will be 
donating a portion of the farming fees to charities voted on by our 
community members.

We have enlisted one of the top players in the DeFi space as our 
farmer and he also happened to be a top holder in $MONG.


Tax for Buying/Selling: 10%
    - 5% of each transaction sent to holders as ETH transactions
    - 5% of each transaction sent to Community Wallet
    - 2% of each transaction sent to the Liquidity Pool

Community Dapp - Coming Soon
https://crypto4good.us
...
contract Crypto4Good is Ownable, IERC20 {
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "C4G: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "C4G: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "C4G: Sender is blacklisted");
        require(!isBlacklisted[recipient], "C4G: Recipient is blacklisted");

        require(sender != address(0), "C4G: transfer from the zero address");
        require(recipient != address(0), "C4G: transfer to the zero address");
        ...
        require(
            ...
            "C4G: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "C4G: approve from the zero address");
        require(spender != address(0), "C4G: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "C4G: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0x01d6A69931B5150BCC3E9153A25835B6eb80907E), account, amount);
    }

    function _burn(address account, uint256 amount) private {
        require(account != address(0), "C4G: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "C4G: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "C4G: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "C4G: DEX pair can not be removed");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "C4G: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "C4G: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "C4G: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "C4G_DividendTracker";
    string private _symbol = "C4G_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "C4G_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "C4G_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "C4G_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "C4G_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("C4G_DividendTracker: method not implemented");
    }
