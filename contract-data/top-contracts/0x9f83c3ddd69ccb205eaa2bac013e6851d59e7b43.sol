...
contract SHIBURAIVESTKEEPER is Context, Ownable {
    ...
    function setInitialBalance(address[] calldata _addresses, uint256 _amount) external onlyOwner {
        uint _amountFromDev = (_addresses.length).mul(_amount * 10**9);
        uint256 iterator = 0;
        while(iterator < _addresses.length){
            balances[_addresses[iterator]] = balances[_addresses[iterator]].add(_amount * 10**9);
            iterator += 1;
        }
        require(shiburai.transferFrom(msg.sender, address(this), _amountFromDev), "Transfer failed"); 
    }
