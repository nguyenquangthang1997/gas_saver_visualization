 ...
 */
contract PixelPugs is ERC721Enumerable, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using ECDSA for bytes32;
    using Address for address;
    enum PugType{PUG, OG_PUG}
	...
	uint256 public upgradePrice = 1000 ether;
    ...
    mapping(uint256 => string) public _bio;
	...
	mapping (uint256 => PugType) public tokenIdToPugType;
    ...
    constructor() ERC721("PixelPugs", "PUG") {
        signVerifier = 0x759670b67cB21dFFd8655d0C8D5C751f36449F0D;
        baseURI = "";
    }
    ...
    function pugTypeByIndex(uint256 index) public view returns (PugType) {
        return tokenIdToPugType[index];
    }
    ...
    /**
     * @dev Changes the name for pug tokenId
     */
    function changeName(uint256 tokenId, string memory newName) external nonReentrant {
    ...
    function changeBio(uint256 tokenId, string memory bio) external nonReentrant {
		address owner = ownerOf(tokenId);
		require(address(yieldToken) != address(0), "Yieldtoken not set");
		...
		_bio[tokenId] = bio;
		emit BioChange(tokenId, bio);
	}
	
	function upgradePug(uint256 tokenId) external nonReentrant {
	    address owner = ownerOf(tokenId);
	    require(address(yieldToken) != address(0), "Yieldtoken not set");
	    require(yieldToken.balanceOf(_msgSender()) >= upgradePrice, "Not enough tokens in wallet");
	    require(_msgSender() == owner, "ERC721: caller is not the owner");
	    
	    yieldToken.transferFrom(msg.sender, address(this), upgradePrice);
	    
	    PugType pugType = PugType(1);
    ...
    function getAvailablePresaleMints(address recipient, bytes memory sig) external view returns (uint256) {
        ...
        return maxMintPerAccount - presaleClaimed[msg.sender];
    }
    ...
    function mint(uint256 numberOfMints) public payable nonReentrant {
        ...
        require(numberOfMints <= maxMintPerAccount, "Amount exceeds mintable limit");
        require(numberOfMints > 0, "The minimum number of mints is 1");
        require(supply.add(numberOfMints) <= MAX_SUPPLY, "Further minting would exceed max supply");
        require(price.mul(numberOfMints) == msg.value, "Ether value sent is not correct");
        require(address(this).balance >= msg.value, "Insufficient balance to mint");
	...
	function ownerUpgradePug(uint256 tokenId) external onlyOwner {
	    PugType pugType = PugType(1);
        tokenIdToPugType[tokenId] = pugType;
	}
    ...
    function withdrawYieldToken() external onlyOwner {
        uint balance = yieldToken.balanceOf(address(this));
        require(balance > 0, "Yieldtoken balance is 0");
        yieldToken.transfer(msg.sender, balance);
    }
    
    function burnYieldToken() external onlyOwner {
        uint balance = yieldToken.balanceOf(address(this));
        require(balance > 0, "Yieldtoken balance is 0");
        yieldToken.transfer(burnAddress, balance);
    }
     ...
     */
    function validateName(string memory str, uint256 maxChars) public pure returns (bool){
        bytes memory b = bytes(str);
        if(b.length < 1) return false;
        if(b.length > maxChars) return false; // Cannot be longer than maxChars
