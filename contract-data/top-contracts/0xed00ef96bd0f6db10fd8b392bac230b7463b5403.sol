...
// GO TO LINE 1904 TO SEE WHERE THE KITTY CONTRACT STARTS
...
// This contract is based on BoringBananasCo to make it easier!


pragma solidity ^0.7.0;
pragma abicoder v2;

contract PixelKitties is ERC721, Ownable {

    using SafeMath for uint256;

    string public BANANA_PROVENANCE = ""; // IPFS URL WILL BE ADDED WHEN KITTIES ARE ALL SOLD OUT

    string public LICENSE_TEXT = ""; // IT IS WHAT IT SAYS

    bool licenseLocked = false; // TEAM CAN'T EDIT THE LICENSE AFTER THIS GETS TRUE

    uint256 public constant bananaPrice = 30000000000000000; // 0.03 ETH
    ...
    // Reserve 125 Kitties for team - Giveaways/Prizes etc
    ...
    constructor() ERC721("Pixel Kitty Supercar Club", "PKSC") { }
    ...
    function tokenLicense(uint _id) public view returns(string memory) {
        require(_id < totalSupply(), "CHOOSE A KITTY WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function mintBoringBanana(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint a Kitty");
        require(numberOfTokens > 0 && numberOfTokens <= maxBananaPurchase, "Can only mint 20 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_BANANAS, "Purchase would exceed max supply of Kitties");
    ...
    function changeBananaName(uint _tokenId, string memory _name) public {
        require(ownerOf(_tokenId) == msg.sender, "Hey, your wallet doesn't own this kitty!");
    ...
    function viewBananaName(uint _tokenId) public view returns( string memory ){
        require( _tokenId < totalSupply(), "Choose a kitty within range" );
        return bananaNames[_tokenId];
    }


    // GET ALL KITTIES OF A WALLET AS AN ARRAY OF STRINGS. WOULD BE BETTER MAYBE IF IT RETURNED A STRUCT WITH ID-NAME MATCH
