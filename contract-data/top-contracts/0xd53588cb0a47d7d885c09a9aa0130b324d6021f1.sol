...
contract DoodleThieves is ERC721("Doodle Thieves", "DT") {
  ...
  uint256 public constant totalSupply = 7_777;
  ...
  function claimTokens(uint256 _amount) external payable tokensAvailable(_amount) {
        require(
          isSaleActive,
          "Sale not started"
        );
        require(circulatingSupply + _amount <= 1777, "Free claiming ended.");
        require(_amount > 0 && _amount <= 2, "Mint min 1, max 2");
        require(balanceOf(msg.sender) + _amount <= 2, "You can claim only 2 free tokens");
