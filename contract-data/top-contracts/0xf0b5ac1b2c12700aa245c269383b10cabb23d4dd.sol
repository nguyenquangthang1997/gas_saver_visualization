...
// File contracts/DoodLadsNFT.sol
...
/**
    Dood Lads is a collection of 4,555 randomly generated NFTs on the Ethereum blockchain.
     A tribute project inspired by Larva Lads and Doodles.
    Join the Doodlelads and vibe with other Dood Lads!
    Spread our !vibe over the whole space!
    Make friends here! WGMI!
**/
contract DoodLadsNFT is ERC721, Ownable {
    /// @dev Library
    ////////////////////////////////////////////
    ...
    ////////////////////////////////////////////
    uint256 public constant TOTAL_SUPPLY = 3555;

    uint256 public constant PRICE = 0.0188 ether;

    uint256 public constant PRE_SAlE_AMOUNT = 1000;

    uint256 public constant PRE_SALE_PRICE = 0.0 ether;

    address public constant WITHDRAW_ADDRESS = 0x8c89031Ee2ED29b5d4dc1671078a122f3E000310;
    ...
    string private BASE_URI = "https://nft.doodladsnft.com/id/";
    ...
    /// NFT Related
    ////////////////////////////////////////////////////////////////////////
    constructor() ERC721("DooDLads", "Lads") {}
    ...
    function queryPurchaseLimit(address target) public view returns (uint256) {
        if (totalSold() >= PRE_SAlE_AMOUNT) {
            return 10;
        } else {
            return 5;
        }
    ...
    function queryPurchaseTotalFee(uint256 amount) public view returns (uint256 totalFee) {
        ...
        if (totalSold() >= PRE_SAlE_AMOUNT) {
            ...
            if (totalSold() + amount > PRE_SAlE_AMOUNT) {
                totalFee = (PRE_SAlE_AMOUNT - totalSold()) * PRE_SALE_PRICE + (PRICE * (amount - (PRE_SAlE_AMOUNT - totalSold())));
            } else {
                totalFee = (PRE_SALE_PRICE * amount);
            }
        }
    }
    ...
    function purchaseValidate(address purchaseUser, uint256 amount) private view {
        ...
        if (totalSold() > PRE_SAlE_AMOUNT) {
            ...
            if (totalSold() + amount > PRE_SAlE_AMOUNT) {
                uint256 requireAmount = (PRE_SAlE_AMOUNT - totalSold()) * PRE_SALE_PRICE + (PRICE * (amount - (PRE_SAlE_AMOUNT - totalSold())));
                require(msg.value >= requireAmount, "insufficient value");
            } else {
                // preSales
                // require(ownedNFTs[purchaseUser].length() + amount <= 3, "purchase over is limite");
                require(amount <= 5, "at most purchase 5");
                require(msg.value >= (PRE_SALE_PRICE * amount), "insufficient value");
            }
        }
    }
    ...
    function listMyNFT(address owner) external view returns (uint256[] memory tokens) {
        EnumerableSet.UintSet storage nftSets = ownedNFTs[owner];
    ...
    function setSelling(bool isSelling) external onlyOwner {
