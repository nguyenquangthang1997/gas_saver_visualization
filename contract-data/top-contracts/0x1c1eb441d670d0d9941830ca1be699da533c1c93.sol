...
A factory for creating mintable ERC721 tokens with mint-to-print utility, based on Zora's ERC721.
https://mintroad.xyz
v1.0.0 (wip)

*/

/// @title: Mint ⧫ Road Factory Contract
/// @author iain nash, george, tzmartin
    ...
    OwnableUpgradeable
{
     ...
     */
     // NOTE[@george]:original above, attempting to allow curator and owner to withdraw fees in a simular way auction house ends auction.
    function withdraw() external {
        require(msg.sender == owner() || msg.sender == curator.curator, "Caller is not owner or curator");

        if (curator.curator != address(0)) {
            uint256 balance = address(this).balance;
            uint256 curatorFee = balance.mul(curator.feePercentage).div(100);
            uint256 remainingBalance = balance.sub(curatorFee);

            AddressUpgradeable.sendValue(payable(curator.curator), curatorFee);
            AddressUpgradeable.sendValue(payable(owner()), remainingBalance);
