...
import "./GB721Enumerable.sol";

abstract contract GB721Batch is GB721Enumerable, IERC721Batch {
  function isOwnerOf( address account, uint[] calldata tokenIds ) external view override returns( bool ){
    for(uint i; i < tokenIds.length; ++i ){
      if( account != owners[ tokenIds[i] ] )
        return false;
    }
...
abstract contract GB721 is Context, ERC165, IERC721, IERC721Metadata {
  ...
  function balanceOf(address owner) public view override returns (uint balance){
    require(owner != address(0), "GB721: query for the zero address");
  ...
  function ownerOf(uint tokenId) public view override returns( address owner ){
    require(_exists(tokenId), "GB721: query for nonexistent token");
  ...
  function approve(address to, uint tokenId) external override {
    ...
    require(to != owner, "GB721: approval to current owner");

    require(
      _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
      "GB721: caller is not owner nor approved for all"
    );
  ...
  function safeTransferFrom(address from, address to, uint tokenId, bytes memory _data) public override {
    require(_isApprovedOrOwner(_msgSender(), tokenId), "GB721: caller is not owner nor approved");
  ...
  function _checkOnERC721Received(address from, address to, uint tokenId, bytes memory _data)private returns ( bool ){
    if (to.isContract()) {
      try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
        ...
        if (reason.length == 0) {
          revert("GB721: transfer to non ERC721Receiver implementer");
        } else {
          assembly {
  ...
  function _exists(uint tokenId) internal view returns ( bool ){
    return tokenId < owners.length && owners[tokenId] != address(0);
  }
  ...
  function _safeTransfer(address from, address to, uint tokenId, bytes memory _data) internal{
    _transfer(from, to, tokenId);
    require(_checkOnERC721Received(from, to, tokenId, _data), "GB721: transfer to non ERC721Receiver implementer");
  }

  function _transfer(address from, address to, uint tokenId) internal {
    require(ownerOf(tokenId) == from, "GB721: transfer of token that is not own");
