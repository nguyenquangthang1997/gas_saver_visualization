...
contract Avatars is Ownable, ERC721 {
    ...
    uint256 public NFT_PRICE = 0; // yay!
    ...
    // Which Original Crypterior has been used for mint and for how many times in a particular wallet
    ...
    function mintNFT(uint256 numberOfNfts) external {
        ...
        require(numberOfNfts <= 2, "You may not buy more than 2 NFTs at once");
        require(totalPublicSupply + numberOfNfts <= LOCALS_MAX, 'Purchase would exceed LOCALS_MAX');

        require(_publicSaleClaimed[msg.sender] + numberOfNfts <= publicSaleMaxMint, 'Purchase exceeds max allowed');

        for (uint256 i = 0; i < numberOfNfts; i++) {
            if (totalPublicSupply < LOCALS_MAX) {
                uint256 tokenId = LOCALS_RESERVE + totalPublicSupply + 1;

                totalPublicSupply += 1;

                _publicSaleClaimed[msg.sender] += 1;
    ...
    {
        return v1TokenIdUsedForMint[msg.sender][_tokenId];
    }
    ...
    function mintForOriginalCrypteriors(uint256[] calldata tokenIds) external {
        require( hasPrivateSaleStarted == true, "Sale has not started");

        require( tokenIds.length < 10, "Cannot mint more than 10 in one transaction!");
        require( tokenIds.length > 0,  "Number of tokens can not be less than or equal to 0");        
        require(totalPublicSupply + tokenIds.length <= LOCALS_MAX, 'Purchase would exceed LOCALS_MAX');
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(v1TokenIdUsedForMint[msg.sender][tokenIds[i]] < MAX_MINTS_PER_ORIGINAL_CRYPTERIOR, "This Crypterior already been used for a free mint in your wallet");
            
            v1TokenIdUsedForMint[msg.sender][tokenIds[i]] += 1;
    ...
    function mintReserve(uint256 _count, address _to) public onlyOwner {
        require(totalSupply() < MAX_NFT_SUPPLY, 'All tokens have been minted');
        require(totalGiftSupply + _count <= LOCALS_RESERVE, 'Not enough tokens left to reserve');
    ...
    */
        ...
        returns (uint256[] memory)
    {
        ...
        if (tokenCount == 0) {            
            ...
            for (tokenId = 0; tokenId < (LOCALS_RESERVE + totalPublicSupply + 1); tokenId++) {
                if ( _exists(tokenId) ) {
