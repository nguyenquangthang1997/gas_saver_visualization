...
contract VikingClub is ERC721Enumerable, Ownable {
    using Strings for uint256;
    using ECDSA for bytes32;

    uint256 public TVC_MAX = 1500;
    uint256 public TVC_PRICE = 0.05 ether;
    uint256 public TVC_MINT = 2;
    uint256 public phase = 1;
    
    //mapping(address => uint256) public listPurchases;
    mapping(string => bool) private _usedNonces;

    mapping(uint256 => mapping(address => uint256)) public listPurchases;
    ...
    address private _signerAddress = 0x12342b9f93DA1bD4b0154eB8A033650CC1b4ba34;
    ...
    constructor() ERC721("The Viking Club", "TVC") { }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= TVC_MAX, "MAX_MINT");
     ...
     function claimReserved(address recipient, uint256 amount) external onlyOwner {
        require(totalSupply() + amount <= TVC_MAX, "MAX_MINT");
    ...
    function withdraw() external onlyOwner {
        payable(_devAddress).transfer(address(this).balance / 5);
        payable(msg.sender).transfer(address(this).balance);
    }
    
    function purchasedCount(address addr) external view returns (uint256) {
        return listPurchases[phase][addr];
    }

     function purchasedTotal(address addr) external view returns (uint256) {
        uint256  purchases = 0;
        for (uint256 i = 1; i <= phase; i++) {
            purchases = purchases + listPurchases[i][addr];
        }
        return purchases;
    }
    ...
    function setPhase(uint256 max, uint256 price, uint256 mint, uint256 ph) external onlyOwner notLocked {
        TVC_MAX = max;
        TVC_PRICE = price;
        TVC_MINT = mint;
        phase = ph;
