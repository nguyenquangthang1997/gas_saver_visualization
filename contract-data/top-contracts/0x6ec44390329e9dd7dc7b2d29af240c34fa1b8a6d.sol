 ...
 */
contract HLFamily is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public START_MINT;

    uint256 public MINT_KEEP_SEC;
    ...
    constructor(
        ...
        uint256 mintStart,
        uint256 mintKeepMinutes
    ) ERC721(name, symbol) {
        _setBaseURI(baseURI);
        MAX_NFTS = maxNftSupply;
        START_MINT = mintStart;
        MINT_KEEP_SEC = mintKeepMinutes * 60;
    }
     ...
     */
    function setStartMint(uint256 mintStart, uint256 mintKeepMinutes)
        public
        onlyOwner
    {
        START_MINT = mintStart;
    ...
    function mint(uint256 numberOfTokens) public payable {
        require(block.timestamp > START_MINT, "Has not yet started to mint");
        require(
            MINT_KEEP_SEC == 0 ||
                block.timestamp < (START_MINT + MINT_KEEP_SEC),
            "Minting has stopped"
        );
        require(
            numberOfTokens <= maxPurchase,
            "Can only mint limited tokens at a time"
        );
        require(
