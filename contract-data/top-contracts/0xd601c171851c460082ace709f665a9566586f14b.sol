...
contract HewerClan is ERC721, Ownable {
    using SafeMath for uint256;

    uint constant public MAX_PURCHASE = 31;
    uint constant public MAX_NUM_OF_TAGS = 10001;
    uint constant public MAX_NUM_OF_PRESALE_TAGS = 2001;
    uint256 constant public TAG_PRICE = 0.06 ether;
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 communityShare = balance.mul(2).div(5);
        uint256 teamShare = balance.sub(communityShare);
    ...
    function reserveHewerTags(address _to, uint amount) public onlyOwner {
    ...
    function mintHewerTags(uint numOfTags) public payable {
        ...
        require(total.add(numOfTags) < MAX_NUM_OF_TAGS, "Cannot exceed max number of tags");
        require(TAG_PRICE.mul(numOfTags) == msg.value, "Ether value sent is not correct");
    ...
    function presaleMintHewerTags(uint numOfTags) public payable {
        require(presaleIsActive, "Presale is not active");
        require(whitelist[msg.sender].isApproved, "You are not in the whitelist to mint a Hewer Tag");
        require(numOfTags > 0, "Must at least mint 1 tag");
        require(numOfTags < MAX_PURCHASE, "Cannot mint more than 30 tags at once");
