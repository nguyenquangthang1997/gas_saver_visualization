/**
    ...
    Right from outer space. Liquidity protocol with ETH reflections.

    🌍 Website: https://www.hubbleinu.com/

    🐦 Twitter: https://twitter.com/HubbleInuToken

    🗣 Telegram: https://t.me/HubbleInuToken
*/

// Verified using https://dapp.tools

// hevm: flattened sources of src/HUBBLE.sol
...
Tax for Buying/Selling: 10%
    ...
    - 6.9% of each transaction sent to Treasury Wallet
    - 0.1% liquidity pool (price floor)
*/
...
contract HUBBLE is Ownable, IERC20 {
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "HUBBLE: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "HUBBLE: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "HUBBLE: Sender is blacklisted");
        require(!isBlacklisted[recipient], "HUBBLE: Recipient is blacklisted");

        require(sender != address(0), "HUBBLE: transfer from the zero address");
        require(recipient != address(0), "HUBBLE: transfer to the zero address");
        ...
        require(
            ...
            "HUBBLE: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "HUBBLE: approve from the zero address");
        require(spender != address(0), "HUBBLE: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "HUBBLE: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "HUBBLE: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "HUBBLE: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "HUBBLE: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "HUBBLE: DEX pair can not be removed");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "HUBBLE: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "HUBBLE: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "HUBBLE: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "HUBBLE_DividendTracker";
    string private _symbol = "HUBBLE_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "HUBBLE_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "HUBBLE_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "HUBBLE_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "HUBBLE_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("HUBBLE_DividendTracker: method not implemented");
    }
