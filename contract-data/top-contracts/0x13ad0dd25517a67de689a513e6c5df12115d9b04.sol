...
import "./BD721Enumerable.sol";

abstract contract BD721Batch is BD721Enumerable, IERC721Batch {
...
abstract contract BD721 is Context, ERC165, IERC721, IERC721Metadata {
  ...
  function ownerOf(uint tokenId) public view override returns( address owner ){
    require(_exists(tokenId), "BD721: query for nonexistent token");
  ...
  function approve(address to, uint tokenId) external override {
    ...
    require(to != owner, "BD721: approval to current owner");

    require(
      _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
      "BD721: caller is not owner nor approved for all"
    );
  ...
  function safeTransferFrom(address from, address to, uint tokenId, bytes memory _data) public override {
    require(_isApprovedOrOwner(_msgSender(), tokenId), "BD721: caller is not owner nor approved");
  ...
  function _checkOnERC721Received(address from, address to, uint tokenId, bytes memory _data)private returns ( bool ){
    if (to.isContract()) {
      try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
        ...
        if (reason.length == 0) {
          revert("BD721: transfer to non ERC721Receiver implementer");
        } else {
          assembly {
  ...
  function _safeTransfer(address from, address to, uint tokenId, bytes memory _data) internal{
    _transfer(from, to, tokenId);
    require(_checkOnERC721Received(from, to, tokenId, _data), "BD721: transfer to non ERC721Receiver implementer");
  }

  function _transfer(address from, address to, uint tokenId) internal {
    require(ownerOf(tokenId) == from, "BD721: transfer of token that is not own");
