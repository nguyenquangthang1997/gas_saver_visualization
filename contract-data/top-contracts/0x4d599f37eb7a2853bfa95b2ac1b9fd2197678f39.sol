...
contract PunkAsses is ERC721,ERC721Enumerable, Ownable {

    InterfaceAss public Ass;
    ...
    mapping (address => uint256) public punkassBalance;

    // withdraw addresses
    address f1 = 0xb4a20CE1411F19397d94B940B85C7148123110c8;
    address f2 = 0x854518B5a4b53c14458C703F13B849C012185837;
    ...
    function setAss(address _ass) external onlyOwner { 
         Ass = InterfaceAss(_ass); 
    }

    function PunkAssMint(uint256 num) public payable {
        uint256 supply = totalSupply(); 
        require( !_paused,                      "Minting paused" );
        require( num < 31,                      "You can mint a maximum of 30 Asses" );
        require( supply + num < 10001,          "Exceeds maximum Punk Ass supply" );
        require( msg.value >= _price * num,     "Eth sent is not correct" ); 
        for(uint256 i; i < num; i++){
            _safeMint( msg.sender, supply + i );
            punkassBalance[msg.sender]++;
        }
    }

    function giveAway(address _to, uint256 _amount) external onlyOwner() {
        uint256 supply = totalSupply(); 
        require( supply + _amount < 10001,  "Exceeds maximum Punk Ass supply" );
