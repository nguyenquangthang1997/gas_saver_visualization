...
// File: contracts/ToadzWorld.sol

pragma solidity >=0.6.0 <0.8.0;

/**
 * @title Toadz World contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract ToadzWorld is ERC721, Ownable, PaymentSplitter {
    
    bool public onlyToadz = true;
    ...
    uint256 public tokenPrice = 36900000000000000;
    ...
    mapping (address => bool) earlyAddr;
    
    mapping (address => bool) explorerAddr;
    
    mapping (address => bool) lateAddr;
    
    //Parent NFT Contract
    //mainnet address
	address public nftAddress = 0x1CB1A5e65610AEFF2551A50f76a87a7d3fB649C6;
	// rinkeby toadz address
    ...
    constructor() public ERC721("ToadzWorld", "TWORLD") {}
    ...
    function flipOnlyToadzState() public onlyOwner {
        onlyToadz = !onlyToadz;
    }
    ...
    function mintSpecial(uint256 [] memory devIDs) external onlyOwner {        
    ...
    function multiMintWithnft(uint256 [] memory nftIds) external {
    ...
    function mintEarly(uint256 quantity) external payable {
        require(tokenCount - 1 + quantity <= maxTokens, "minting this would exceed supply");
        require(msg.value >= tokenPrice, "not enough ether sent!");
        require(msg.sender == tx.origin, "no contracts please!");
        require(openMint, "mint not open");
        require(earlyAddr[msg.sender], "you're not on the whitelist");
        require(onlyWhitelisted, "whitelist not open yet");
        require(quantity <= 3, "trying to mint too many at a time");
        for (uint256 i = 0; i < quantity; i++) {
            if(_exists(tokenCount)) {
                tokenCount++;
                i--;
                continue;
            }
    ...
    function mintExplorer(uint256 quantity) external payable {
        require(tokenCount - 1 + quantity <= maxTokens, "minting this would exceed supply");
        require(msg.value >= tokenPrice, "not enough ether sent!");
        require(msg.sender == tx.origin, "no contracts please!");
        require(openMint, "mint not open");
        require(explorerAddr[msg.sender], "you're not on the whitelist");
        require(onlyWhitelisted, "whitelist not open yet");
        require(quantity <= 2, "trying to mint too many at a time");
        for (uint256 i = 0; i < quantity; i++) {
            if(_exists(tokenCount)) {
                tokenCount++;
                i--;
    ...
    function mintLate(uint256 quantity) external payable {
        require(tokenCount - 1 + quantity <= maxTokens, "minting this would exceed supply");
        require(msg.value >= tokenPrice, "not enough ether sent!");
        require(msg.sender == tx.origin, "no contracts please!");
        require(openMint, "mint not open");
        require(lateAddr[msg.sender], "you're not on the whitelist");
        require(onlyWhitelisted, "whitelist not open yet");
        require(quantity == 1, "trying to mint too many at a time");
        for (uint256 i = 0; i < quantity; i++) {
            if(_exists(tokenCount)) {
                tokenCount++;
                i--;
    ...
    function setEarlyWhitelistUsers(address[] calldata _users) public onlyOwner {
        for (uint i = 0; i < _users.length; i++) {
            earlyAddr[_users[i]] = true;
        }
    }
    
    
    function setExplorerWhitelistUsers(address[] calldata _users) public onlyOwner {
        for (uint i = 0; i < _users.length; i++) {
            explorerAddr[_users[i]] = true;
        }
    }
    
    function setLateWhitelistUsers(address[] calldata _users) public onlyOwner {
        for (uint i = 0; i < _users.length; i++) {
            lateAddr[_users[i]] = true;
        }
    }
    
    
    function isEarlyWhitelisted(address _user) public view returns (bool) {
        return earlyAddr[_user];
    }
    
    function isLateWhitelisted(address _user) public view returns (bool) {
        return lateAddr[_user];
    }
    
    function isExplorerWhitelisted(address _user) public view returns (bool) {
        return explorerAddr[_user];
    }
