...
contract WizardStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    mapping(uint256 => address) private _tokenErc20;
    ...
    // Set this to update the ERC20 address to receive from staking
    function setERC20Address(address _erc20address) public onlyOwner {
        require(
            erc20Address == address(0),
            "Staking: ERC20 address already set"
        );
        erc20Address = _erc20address;
    }
    ...
    function balanceOf(address account) external view returns (uint256) {
        return _deposits[account].length();
    }
    ...
    function deposit(uint256[] calldata tokenIds) external whenNotPaused {
        require(msg.sender != stakingToken, "Invalid address");
    ...
    function autoStake(address from, uint256 tokenId) external {
        require(msg.sender == stakingToken, "Invalid address");
        require(
            IERC721(stakingToken).ownerOf(tokenId) == address(this),
            "Invalid tokenId"
        );
        _deposits[from].add(tokenId);
        _depositBlocks[from][tokenId] = Math.min(block.number, expiration);
    }
