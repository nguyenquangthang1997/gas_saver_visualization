...
contract StacyVaultRewardsLock is Ownable {
    using SafeMath for uint256;
    ...
    constructor(address _stacy, uint256 _lockFromBlock, uint256 _lockToBlock)
    public
    Ownable()
    {
        stacy = IERC20(_stacy != address(0) ? _stacy : 0xf12EC0D3Dab64DdEfBdC96474bDe25af3FE1B327); // For testing
    ...
    function lock(address _holder, uint256 _amount) public /*onlyOwner*/ {
        ...
        stacy.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function unlock() public {
        require(_locks[msg.sender] > 0, "cannot unlock");
        
        uint256 amount = canUnlockAmount(msg.sender);

        if (amount > stacy.balanceOf(address(this))) {
            amount = stacy.balanceOf(address(this));
        }

        stacy.safeTransfer(msg.sender, amount);
    ...
    function totalBalanceOf(address _holder) public view returns (uint256) {
        return _locks[_holder].add(stacy.balanceOf(_holder));
    }
