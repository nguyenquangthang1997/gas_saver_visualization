    ...
    PaymentSplitter
{
    ...
    uint16 public maxSaleSupply = 9999;
    uint16 public maxPresaleSupply = 2000;
    uint16 public maxGiftSupply = 100;
    uint256 public presalePrice = 200000000000000000; //0.2 ETH
    uint256 public publicPrice = 250000000000000000; //0.25 ETH
    uint16 public giftCount;
    uint16 public presaleCount;
    uint16 public totalNFT;
    ...
    address[] private _team = [
        0x18916B6C95f70eBa01691f8c0E012A25D10D88FD,
        0xdaAa999893d6f960EDa2a9Fef41eb837aeFbCd8D,
        0x9b9a030FEB6ed824b2139FCAea41cB7E9E24aD03,
        0x1493802B39D454f62Aa400dFfBF4838eE9b92AE1
    ];
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Metanauts: Can't add a zero address"
            );
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Metanauts: Can't remove a zero address"
            );
    ...
    function setUpPresale(uint256 _duration) external onlyOwner {
        require(
            ...
            "Metanauts: Unauthorized Transaction"
        );
    ...
    function setUpSale() external onlyOwner {
        ...
        require(
            ...
            "Metanauts: Sale not started"
        );
        uint256 _startTime = block.timestamp;
        uint256 _maxCount = 100;
    ...
    function giftMint(address[] calldata _addresses) external onlyOwner {
        require(
            ...
            "Metanauts: max total supply exceeded"
        );

        require(
            giftCount + _addresses.length <= maxGiftSupply,
            "Metanauts: max gift supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Metanauts: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function presaleMint(uint256 _amount) external payable nonReentrant {
        ...
        require(
            _amount <= 2,
            "Metanauts: You can't mint so much tokens"
        );
        require(
            _presaleClaimed[msg.sender] + _amount <= 2,
            "Metanauts: You can't mint so much tokens"
        );
        require(
            _presaleList[msg.sender] == true,
            " Metanauts: Caller is not on the presale list"
        );
        require(
            _presaleConfig.startTime > 0,
            "Metanauts: Presale must be active to mint a NFT"
        );
        require(
            block.timestamp >= _presaleConfig.startTime,
            "Metanauts: Presale not started"
        );
        require(
            block.timestamp <=
                _presaleConfig.startTime + _presaleConfig.duration,
            "Metanauts: Presale is ended"
        );
        require(
            totalNFT + _amount <= maxPresaleSupply,
            "Metanauts: max presale supply exceeded"
        );
        require(
            totalNFT + _amount <= maxSupply,
            "Metanauts: max supply exceeded"
        );
        require(
            presalePrice * _amount <= msg.value,
            "Metanauts: Ether value sent is not correct"
        );
        require(!paused, "Metanauts: contract is paused");
        uint256 _newItemId;
    ...
    function saleMint(uint256 _amount) external payable nonReentrant {
        ...
        require(_amount > 0, "Metanauts: zero amount");
        require(
            _saleConfig.startTime > 0,
            "Metanauts: sale is not active"
        );
        require(
        ...
        emit SaleMint(msg.sender, _amount, publicPrice);
    ...
    function changeTotalSupply(uint16 _newSupply) public onlyOwner {
        maxSupply = _newSupply;
    }

    function changeSaleSupply(uint16 _newSaleSupply) public onlyOwner {
        maxSaleSupply = _newSaleSupply;
    }

    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Metanauts: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Metanauts: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }

    function changePublicPrice(uint16 _newPublicPrice) public onlyOwner {
