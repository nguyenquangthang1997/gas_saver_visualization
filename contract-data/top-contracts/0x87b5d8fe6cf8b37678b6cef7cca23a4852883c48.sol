...
contract EightEightEight is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant RESERVE_SUPPLY = 1;
    uint256 public constant MAX_PUBLIC_MINT = 400;
    uint256 public constant PRICE_PER_TOKEN = 0 ether;

    mapping(address => uint16) private _allowList;

    constructor() ERC721("EightEightEight", "888") {
    }
    ...
    function setAllowList(address[] calldata addresses, uint16[] calldata numAllowedToMint) external onlyOwner {
        require(addresses.length == numAllowedToMint.length, "Lengths must be the same");
        for (uint256 i = 0; i < addresses.length; i++) {
            _allowList[addresses[i]] = numAllowedToMint[i];
        }
    }

    function setAllowList(address[] calldata addresses, uint16 numAllowedToMint) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
