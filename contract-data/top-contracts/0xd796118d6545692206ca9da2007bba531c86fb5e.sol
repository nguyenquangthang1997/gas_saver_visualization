...
Twitter: www.twitter.com/abbyinuofficial
Website: www.abbyinu.com

** Secured With FTP Antibot **
** Using FTP EthReflect to give 2.00% of ALL transactions to holders. **
...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0xF4798E77c56d6Df166603f7A32a45FAD9A2489d0), 5000);
        m_DidDeploy = true;
    }
...
contract AbbyInu is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 1000000000000 * 10**9;
    string private m_Name = "Abby Inu";
    string private m_Symbol = "ABBY";
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != address(0) && _recipient != m_UniswapV2Pair && block.timestamp <= m_Launched.add(20000000 hours);
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(20000000 minutes))
    ...
    function addLiquidity() external onlyOwner() {
		...
		EthReflect.init(address(this), 2000, m_UniswapV2Pair, m_UniswapV2Router.WETH(), _ethBalance, TOTAL_SUPPLY);
