...
/**
 *Submitted for verification at Etherscan.io on 2022-1-24
*/
...
contract Para is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0xf1A16321438a3efCa65A11Ee8163a0fB0bd19cdB;
    address public burnWallet = 0x6d0102234Cfb94fBB5c140750B6944A55654375B;
    address public charityWallet = 0x597bDD598a6901377d1965d0869D731515bF218e;
    address public devWallet = 0x317422CBe04605903b0648414F05D1AbB00B7cC3;

    string private _name = "ParaToken";
    ...
    uint256 public marketingFeePercent = 25;
    uint256 public burnFeePercent = 25;
    uint256 public charityFeePercent = 25;
    uint256 public devFeePercent = 12;
    ...
    Taxes public sellTaxes = Taxes(2, 8, 25, 25, 25, 12);
    Taxes public buyTaxes = Taxes(1, 4, 25, 25, 25, 12);
    Taxes public sendTaxes = Taxes(0, 0, 25, 25, 25, 12);
    
    uint256 public _extraFee = 8;
    ...
    function setBurnWallet(address walletAddress) public onlyOwner {
        burnWallet = walletAddress;
    }

    function setCharityWallet(address walletAddress) public onlyOwner {
    ...
    function setSellTaxes(Taxes memory newSellTaxes) external onlyOwner{
        sellTaxes = newSellTaxes;
    }

    function setBuyTaxes(Taxes memory newBuyTaxes) external onlyOwner{
        buyTaxes = newBuyTaxes;
    }

    function setSendTaxes(Taxes memory newSendTaxes) external onlyOwner{
        sendTaxes = newSendTaxes;
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _extraFee == 0) return;
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair) { // Token sell taxes
            _taxFee = sellTaxes.tax;
            _extraFee = sellTaxes.extra;
            marketingFeePercent = sellTaxes.marketing;
            burnFeePercent = sellTaxes.burn;
            charityFeePercent = sellTaxes.charity;
            devFeePercent = sellTaxes.dev;

        } else if (from == uniswapV2Pair) { // Token buy taxes
            _taxFee = buyTaxes.tax;
            _extraFee = buyTaxes.extra;
            marketingFeePercent = buyTaxes.marketing;
            burnFeePercent = buyTaxes.burn;
            charityFeePercent = buyTaxes.charity;
            devFeePercent = buyTaxes.dev;
        } else { // Token send taxes
            _taxFee = sendTaxes.tax;
            _extraFee = sendTaxes.extra;
            marketingFeePercent = sendTaxes.marketing;
            burnFeePercent = sendTaxes.burn;
            charityFeePercent = sendTaxes.charity;
            devFeePercent = sendTaxes.dev;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 burnShare = newBalance.mul(burnFeePercent).div(100);
        payable(burnWallet).transfer(burnShare);
        newBalance -= burnShare;

        uint256 charityShare = newBalance.mul(charityFeePercent).div(100);
        payable(charityWallet).transfer(charityShare);
        newBalance -= charityShare;

        uint256 devShare = newBalance.mul(devFeePercent).div(100);
